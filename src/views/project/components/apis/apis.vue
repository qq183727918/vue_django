  ...
  >
    <el-form ref="forms" :model="form" :rules="rules" label-width="40px">
      <el-form-item>
        ...
        <div class='input' style="width: 80%" :model="form">
          <el-input v-model="input" placeholder='请输入请求地址，如：http://wms.test.vevor.net/log'></el-input>
        </div>
        <div class='button'>
...
<script>
  import { doEdit, SaveApis, DebugApis, sendRequest, Returned } from '@/api/project/apis'
  ...
  export default {
    ...
    created() {
      this.showReturt('radio')
      this.showReturt('select')
      this.showReturt('formArr')
      this.showReturt('formArrx')
      this.showReturt('formArrf')
      this.showReturt('formArrq')
      this.showReturt('ActiveName')
      this.showReturt('input')
      this.showReturt('input1')
      this.showReturt('input2')
      this.showReturt('id')
      this.showReturt('textareas')
      this.showReturt('textarea')
      this.showReturt('values')

    },
    methods: {
      beautiy () {
        this.textareas = JSON.stringify(JSON.parse(this.textareas),null,4)
      },
      tabchange(tab) {
      ...
      showEdits(row) {
        ...
        Returned({ id: row.id }).then(data => {
          const relist = data.data
          this.mock = relist.mock_res
          this.ExpectedResult = relist.expected
          this.AssertPath = relist.assert_path
          this.AssertAll = relist.assert_re
          this.ExtractPath = relist.extract_path
          this.ExtractRe = relist.extract_re
        })
      },
      showReturt(row) {
        console.log(row)
        const dich = new Array()
        const api_bodys = new Array()
        const headers = row.api_header
        
        this.ActiveName = row.api_tag
        if(row.result == undefined){
        }else{
          const str1 = row.result[0]
          const str2 = row.result.substr(row.result.length-1,1)
          if(str1 == '{' && str2 == '}'){
            this.values = JSON.parse(row.result)
          }else{
            this.values = row.result
          }
        }
        // this.textarea = row.result
        for(var s in headers){
          const dicheader = {}
          dicheader['key'] = s,
          dicheader['value'] = headers[s]
          dich.push(dicheader)
        }
        this.formArr = dich
        if(row.api_tag == 'header' || 'body'){
          this.ActiveName = row.api_tag
          if(row.body_method == 'None'){
            this.radio = 1
          }
          if(row.body_method == 'Form-data'){
            this.radio = 2
            const urlform = JSON.parse(row.api_body)
            for(var a in urlform){
              const dicurlform = {}
              dicurlform['key'] = a,
              dicurlform['value'] = urlform[a]
              api_bodys.push(dicurlform)
            }
            this.formArrf = api_bodys
          }
          if(row.body_method == 'X-www-form-urlencoded'){
            this.radio = 3
            const urlform = JSON.parse(row.api_body)
            for(var a in urlform){
              const dicurlform = {}
            ...
            this.formArrx = api_bodys
          }
          if(row.body_method == 'Raw'){
            this.radio = 4
            // this.textareas = JSON.stringify(row.api_body)
            // this.textareas = JSON.stringify(JSON.parse(this.textareas),null,4)
            const rawbody = JSON.stringify(row.api_body)
            const striq1 = rawbody[0]
            const striq2 = rawbody.substr(rawbody.length-1,1)
            if(striq1 == '{' && striq2 == '}'){
              try{
                this.textareas = JSON.stringify(row.api_body)
                this.textareas = JSON.stringify(JSON.parse(this.textareas),null,4)
                ...
                this.textareas = rawbody
              }
            }else{
              this.textareas = rawbody
            }
          }
          if(row.body_method == 'Binary'){
            this.radio = 5
          }
        }
        this.select = {
          options: [
            ...
            value: row.api_models
        },
        this.input = row.api_url
        this.input1 = row.listName
        this.input2 = row.name
        this.id = row.id
        if(row.api_tag == "Query参数"){
          this.ActiveName = "query"
          const query = row.api_body
          for(var s in query){
            const dicquery = {}
            dicquery['key'] = s,
            dicquery['value'] = query[s]
            api_body.push(dicquery)
            this.formArrq = api_body
          }
        }
      },
      close() {
        ...
        this.$refs['forms'].validate(async (valid) => {
         const pro_list = {}
          ...
          if(this.ActiveName == 'body' || 'header'){
            ...
            if(this.radio == 4){
              pro_list['api_body'] = JSON.parse(this.textareas)
            }
...
<style>
  ...
  .jv-node {
    background-color: rgb(231, 236, 182);
  }
